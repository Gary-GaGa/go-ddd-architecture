...
近期已加入多種任務型別（Practice、Deploy、Research），HUD/UX 介面全面升級，底部熱鍵最小化，語言面板排序與商店小卡優化，並移除所有圓角光暈，改用方形設計。
...
- HUD 採用 Renderer 介面，分別實作 Practice、Deploy、Research 三種任務視覺化，底部熱鍵（Hotkeys）最小化並自適應寬度，語言面板支援排序與升級提示，商店小卡改為方形設計。
...
- **系統模組**：
  ...
  - 任務系統（Practice、Deploy、Research 三種型別，支援任務佇列 intent 機制）
- **UI 構想**：主介面、語言學習、技能樹升級、任務日誌、離線收益、底部最小化熱鍵、語言排序、商店小卡、全域方形設計
- **程式語言與技能對應表**：
...
- **放置與解題邏輯**：AI 學會語言後即可自動解題，產生知識點、研發點與特殊事件，離線收益可累積 8 小時。任務進行中按 D/R 會佇列 intent，完成後自動啟動 queued intent。
...
- F/V/K：語言排序（F：循環排序、V：依等級、K：依知識）

任務行為：

- 當沒有進行中任務時，會自動開始練習（Practice）。
- 任務倒數至 0 後，會自動嘗試結算（Try Finish），不用手動按鍵。
- 按 D/R 若有進行中任務則佇列 intent，完成後自動啟動 queued intent。

提示：

- 左側 Status 卡會顯示 Est. Success（估計成功率）、Rates（每分鐘知識/研發產率）。
- 中央任務卡會顯示語言、Base 奬勵與總時長，依任務型別切換 Practice/Deploy/Research 視覺化。
- 中央任務卡背景有淡藍藍圖格線與任務型別標籤（Practice/Deploy/Research），全域改為方形設計。
- 右側 Languages 面板每列提供升級進度條（Research / Next Cost），可升級時顯示 READY 徽章；支援排序與強調色。
- 商店小卡改為方形設計，僅支援滑鼠購買。
- 底部熱鍵（Hotkeys）最小化回歸，依寬度自適應顯示。
